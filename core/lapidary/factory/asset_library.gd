...
static func valid_manifest(value: Variant) -> bool:
	...
	for key: String in value["pages"]:
		...
		if not info is Dictionary or not GemArtifactStore.valid_key(key) or info.get("path") != "pages/" + key + ".gpage" or not info.get("sha256") is String or not GemArtifactStore.valid_key(info["sha256"]) or info.get("codec") != value["codec"]:
			return false
	...
	for key: String in value["frames"]:
		...
		if not GemArtifactStore.valid_key(key) or not item is Dictionary or not value["pages"].has(item.get("page")):
			return false
...
func _page(key: String) -> Texture2D:
	_clock += 1
	if _pages.has(key):
		_pages[key]["used"] = _clock
		return _pages[key]["texture"]
	var info: Dictionary = manifest["pages"].get(key, {})
	if not GemArtifactStore.valid_key(key) or info.is_empty() or info.get("path") != "pages/" + key + ".gpage":
		return null
	...
	if hash.finish().hex_encode() != info["sha256"]:
		return null
	if GemContentIdentity.digest(["delivery-page-v1", Vector2i(info["width"], info["height"]), int(info["format"]), String(info["codec"]), bytes]) != key:
		return null
	var image: Image
