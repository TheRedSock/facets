...
struct VolumeField { vec4 center_absorption; vec4 radius_scatter; vec4 rotation; };
...
struct FieldChord { float closest; float speed2; float height; float begin; float end; };
FieldChord field_chord(VolumeField f, vec3 pos_mm, vec3 velocity_mm, float distance) {
	...
	return FieldChord(closest, speed2, max(0.0, height), max(0.0, closest - span), min(distance, closest + span));
}
...
float chord_column(FieldChord chord, float distance) {
	...
	return half_length * (0.6521451548625461 * (chord_density(chord, midpoint - inner) + chord_density(chord, midpoint + inner))
