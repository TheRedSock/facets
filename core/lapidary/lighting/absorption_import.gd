...
extends RefCounted
## Converts supplied spectroscopy to Napierian absorption /mm before resampling.
...
const UNITS := ["napierian_per_mm", "napierian_per_cm", "napierian_per_m",
	...
	"internal_transmittance", "internal_transmittance_percent", "decadic_absorbance"]
...
static func compile_samples(wavelengths: PackedFloat64Array, ordinary: PackedFloat64Array,
	...
	evidence.assumptions = "Input %s; basis %s; path %.9f mm; coefficient interpolation to 1 nm. %s" % [quantity, basis, length_mm, str(metadata.get("assumptions", ""))]
	...
	for input in [ordinary, extraordinary]:
		if input.is_empty():
			arrays.append(PackedFloat32Array())
			continue
		...
		for value in input:
			...
			if quantity.begins_with("internal_transmittance"):
