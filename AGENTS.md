...
+- **Geometry** — currently a convex plane set, with recession-cone boundedness validation. A procedural mesh/BVH backend is planned; convexity is an optimization, not an engine invariant.
+- **Grading** — cut and homogeneous volume properties are active. Old inclusion primitives and surface wear stay disabled until physical replacements pass validation; do not restore the old primitives by flipping a constant.
