...
- **Tracer** — GLSL compute shader spectral path tracer on `RenderingDevice` (`core/lapidary/tracer/`). No CPU fallback, no denoiser. Kernel v3 is **clean optics**: perfect specular dielectric facets, no surface-wear model. Deterministic Fresnel splitting (transmitted branch evaluates the analytic rig immediately; reflected/TIR branch continues) collapses variance at single-digit spp. Volume scattering is deterministic too: one sampled event, then the path reads a per-frame SH **scatter field** (`gem_scatter_field.glsl` pre-pass) and ends. 4 hero wavelengths share one path; dispersion-strong stones (diamond) split per wavelength. Physics gate: `tools/kernel_physics_check.gd`.
...
- **Inclusions** — analytic primitives (needle, disc, ellipsoid, cloud, veil, fingerprint) seeded from the species vocabulary and `grade.clarity`. Currently **gated off** (`LapidaryStoneCompiler.INCLUSIONS_ENABLED = false`) for the clean-optics baseline; vocabularies stay authored. Flip the const to restore.
...
| Quality per tier | `GemGrade` in `data/lapidary/grades/` (cut/crystal render; `clarity` gated by `INCLUSIONS_ENABLED`; `surface` is data only) |
