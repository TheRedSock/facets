...
- **Grading** — cut and homogeneous volume properties are active. The nonphysical primitive backend is removed. Explicit boundary finishes support light polish; strong frosting and automatic clarity/surface recipes remain unaccepted.
...
- **Lighting language** — `GemLightRig` resources (`data/lapidary/rigs/`), packed by `core/lapidary/lighting/rig_compiler.gd` (`pack()` lights, `environment()` background + Planckian `bg_kelvin` + as-shot `white_kelvin`). Grade is never faked with lighting; lighting changes are rig edits, A/B'd with `tools/rig_ab_check.gd`, designed from the measured face-up return function (`tools/return_sweep.gd`). Exposure is the house print's only; tools and bakes pass 1.0.
- **House print** — mastering pass (`gem_print.glsl` + `GemPrint` resource): spectral → XYZ (2° CMFs) → as-shot white balance (Bradford, rig `white_kelvin` → D65; `core/lapidary/lighting/colorimetry.gd`) → linear sRGB → hue-preserving tonescale / chroma governor → sRGB8, straight alpha. Applied identically to clips and live draws; versioned.
...
| Quality per tier | `GemGrade` in `data/lapidary/grades/` (cut/crystal render; automatic clarity/surface recipes disabled; explicit conditions/finishes supported) |
...
| Trace/shading logic | `core/lapidary/tracer/shaders/gem_pathtrace.glsl` (+ `gem_common.glsl`, `gem_scatter_field.glsl`) + `gem_tracer.gd` host |
...
`GemCondition.finish` and each `GemDefect.finish` hold independent `GemSurface` resources (GGX slope widths and projected polish direction). Binding14 is aligned with the region table. Both reflected and transmitted directions/weights change physically; rough convex hosts retain analytic plane intersections. Light polish passes furnace checks (alpha0.005 loss~0.02%, alpha0.02~0.29% in the quartz fixture). Strong frosting is not accepted: alpha0.3 loses~35% because this single-scattering model omits microfacet multiple scattering. Automatic surface grading stays disabled until more species/rotation checks. Optional reconstruction filters the full rough signal; raw reference accumulations remain available.
