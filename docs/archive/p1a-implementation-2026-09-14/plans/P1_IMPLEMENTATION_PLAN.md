...
Post-implementation amendment: functional evidence remains valid, but normal
sequential per-cell playback regresses fluidity. The
[P1-A amendment](P1_FLUIDITY_AMENDMENT.md) reopens presentation and performance
acceptance and supersedes this plan's sequential-only normal-play checkpoint.
No reactive input or simulation-rule change is adopted.
...
Keep pooled views and existing delivered clips. Retain the sequential projection
for the new facts as a diagnostic reference. P1-A now requires concurrent normal
playback with continuous journeys and verified intermediate paths/causality,
not only equivalent final views. Unproven cross-wave overlap stays disabled;
concurrency inside a settling wave is foundational. Core portal
