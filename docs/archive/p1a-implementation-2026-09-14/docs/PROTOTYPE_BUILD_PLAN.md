...
Status: P1 functional checkpoint verified, 2026-09-14; presentation and performance acceptance reopened. The next implementation is the [P1-A fluidity amendment](../plans/P1_FLUIDITY_AMENDMENT.md), before P2 pacing signoff. The [game design](GAME_DESIGN.md) owns intended behavior. The [rewrite review](GAMEPLAY_REWRITE_REVIEW.md) and [fluidity assessment](FLUIDITY_AND_REACTIVE_PLAY.md) own source findings; [integration](GAME_ENGINE_INTEGRATION.md) and [art review](GEM_ART_REVIEW.md) own delivery/content work. The [content systems design](CONTENT_SYSTEMS.md) owns expansion semantics. See the [P1 handoff](../plans/P1_HANDOFF.md) for implemented scope, checks and measured limitations. This plan authorizes no automatic engine expansion.
...
**Acceptance amendment:** the normal ActionPlayer serializes one-cell falls and
regresses the prior coherent concurrent motion. Its reference implementation is
a migration checkpoint, not sufficient presentation acceptance. P1-A adds actual
action profiling, concurrent full-path playback, lifecycle tests and release
motion review. Keep current state/event goldens and pre-gravity chains unchanged.
Whole-wave match barriers are an accepted first repair; cross-wave overlap is
optional and requires proven dependencies. The 5 ms action target remains open.
...
Optional decision before this phase freezes: the
[reactive-window experiment](FLUIDITY_AND_REACTIVE_PLAY.md) may follow the P2
room under a separate rules profile if explicitly selected. It is not part of
the default expedition. Adoption requires versioned episode/subcommand budgets,
continuations, replay and save-boundary rules before family/reward integration;
otherwise retain the current atomic action and stable-only persistence scope.
...
The next coding task should be **P1-A concurrent motion and measured latency repair**, following its evidence, projection, lifecycle, optimization and release batches. Preserve the completed rule/identity baseline and existing sprites. The single rubble objective room follows for gameplay-feel acceptance; reactive input remains a separately proposed experiment.
