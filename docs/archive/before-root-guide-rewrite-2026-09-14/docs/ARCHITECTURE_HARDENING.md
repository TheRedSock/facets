...
No changes to the frozen P0 snapshot are needed: this audit makes implementation protocols explicit before code lands. The new local-fill portal policy, RNG partition and scheduler version must be recorded in AGENTS.md alongside P1 implementation and new fixtures, not silently treated as existing behavior. Existing engine/delivery contracts remain protected.
