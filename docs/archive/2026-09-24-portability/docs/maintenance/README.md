...
Docs/plans remain gitignored under the existing repository policy. Share this
local package explicitly when moving planned work to another checkout or agent
environment. Updating tracked root links does not make the linked local files
...
new archive manifest hashes. It inventories the two unchanged historical reports
