...
Status: recommended plan, 2026-09-13. The [game design](GAME_DESIGN.md) owns intended behavior. The [rewrite review](GAMEPLAY_REWRITE_REVIEW.md) owns source findings; [integration](GAME_ENGINE_INTEGRATION.md) and [art review](GEM_ART_REVIEW.md) own the delivery/content work. This plan authorizes no automatic engine expansion and claims no new gameplay implementation.
...
- Adopt the proposed three-room experiment as a named rules version. Record decisions for Work/Craft, one gem per rank, family triggers, survivor ordering, carryover and extraction timing.
...
5. Add stable instance/event IDs and canonical complete state serialization/hash. Include topology, obstacles, roster, counters, inventory, RNG state and content/rule version as introduced.
6. Commit a complete action transaction before playback; support cancellation/skip without repeated rule finalization. Keep a compatibility adapter only while callers migrate, with one underlying implementation.
...
1. Curated Quartz/Corundum/Beryl tags and three concrete family reactions. Source family identity from the matched tiles before upgrading. Beryl targets the survivor's post-merge neighborhood; target order and once-per-turn usage are deterministic.
...
5. Commission and flagship extraction, including minimum-rank outlets, dust-ready lock conditions and deterministic post-settle collection.
...
- Test rank numerals, family icons, selection ring and rubble overlays at 112px/80px. Do not rely on facet detail to identify family.
...
- Add the remaining candidate settings and a curated small set of mechanically distinct gem replacements. A new family needs several useful decisions before more artwork.
