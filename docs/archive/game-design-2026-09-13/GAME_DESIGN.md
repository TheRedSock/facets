...
Status: recommended design, 2026-09-13. This is a proposal for the game to build, not a description of implemented gameplay. The user selected a balanced emphasis on family builds and tactical board objectives. Numbers below are initial tuning hypotheses. Current implementation, integration, art review and milestones are in the [rewrite review](GAMEPLAY_REWRITE_REVIEW.md), [integration plan](GAME_ENGINE_INTEGRATION.md), [gem review](GEM_ART_REVIEW.md) and [build plan](PROTOTYPE_BUILD_PLAN.md).
...
| [Balatro](https://store.steampowered.com/app/2379780/Balatro/) | A small familiar grammar becomes expressive through interacting modifiers | Score escalation is optional valuation, not the only room objective |
...
5. **Real gems, fictional systems.** Mineral relationships and appearance have sources. Magical upgrading, rarity bands and monetary values are game rules.
...
## 4. Identity: rank, gem, family, rarity and value

Keep these five concepts separate:

| Concept | Meaning | Consequence |
|---|---|---|
| Rank, 1–8 | Position in the run's merge ladder | Determines shape and next upgrade slot |
| Gem identity | Quartz, amethyst, emerald, etc. | Determines name, art and family membership |
| Mineral family/group | Verified relationship, such as quartz, beryl or corundum | Selects family reactions and rewards |
| Draft rarity | How frequently a reward appears | Controls access, not an assertion of world market price |
| Appraisal value | Integer game value assigned to a rank, optionally modified by explicit settings | Used by selected objectives and run results |

Amethyst is quartz; emerald/aquamarine are beryl; ruby/sapphire are corundum. Garnet and tourmaline are groups rather than one uniform species. Alexandrite belongs to chrysoberyl, not beryl. These relationships must be curated in game metadata, not inferred from names or optical asset IDs. See the [source notes](GEM_ART_REVIEW.md#mineral-and-value-source-notes).

The ladder is a **magical exchange/forging abstraction**, not a geological transformation. The tutorial should state this once. Never describe three amethysts becoming peridot as mineral growth, cutting or heat treatment.

Use rank values `1, 3, 9, 27, 81, 243, 729, 2187` as the first internal valuation table. This reflects merge investment in an ideal three-to-one chain, not dollars or rarity measurements. A later market-themed setting may change appraisal; it must not change match identity.
...
Future settings may move bounded integer weight between slots. Never reduce the prototype to fewer than three spawn ranks, introduce hidden adaptive luck, or draft unrelated additional colors. Record changed weights and RNG state in replay/save data.
...
- Rank-8 matches are terminal collections: remove the matched pieces and record their appraisal, never create rank 9. A rank-8 piece meant for extraction remains a valuable piece until actually consumed.
...
Use a separate cell obstacle layer; do not overload a gem's rank with “T0.” A blocked topology cell is permanent terrain, not a breakable obstacle.
...
Start with six nonstacking settings, maximum three equipped. At capacity, replacing one is explicit. Apply them in fixed slot order; never depend on dictionary iteration.
...
|---|---|---|
| Commission | Extract gems with total appraisal at least the displayed quota, through marked outlets; only ranks at or above the displayed minimum qualify | Learn merge-versus-bank choices |
...
2. **Route choice:** either a 6-rubble seam (16 Work) or a commission (16 Work, minimum extraction rank 3, target appraisal 27, two bottom outlets). Both offer a reward; carry selection after completion.
...
Prototype rewards are free. Appraisal is objective progress/results, **not a second spendable currency**. Gold, silver, dust crafting, shops and risky treatments remain later extensions. This keeps room choices and roster decisions testable without an economy masking their problems.
...
The recommended default is fixed for implementation planning: one gem per rank, three room archetypes, bounded Craft tools, selective carryover and three family reactions. Revisit it using observed play, especially tool dominance, family access and extraction pacing. Later candidates are Garnet adaptation, Tourmaline cascades, a small shop, optional risk/reward inclusions, named challenge routes and a mineral collection log. Advanced optical phenomena, T9 pieces and extra matching grammars are not prerequisites for these.
