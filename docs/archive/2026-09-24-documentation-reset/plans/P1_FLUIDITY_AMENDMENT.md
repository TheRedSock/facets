...
Implementation plan, updated 2026-09-14. The user authorized implementation.
Concurrent normal motion, lifecycle protection, profiling changes and release
checks are delivered; **the 5 ms CPU acceptance gate remains open**. Optional
A4 cross-wave overlap is deferred. See the [P1-A handoff](P1A_HANDOFF.md) for exact
results, recordings and remaining limitations.
The [assessment](../docs/FLUIDITY_AND_REACTIVE_PLAY.md) owns evidence, design
tradeoffs and the reactive-play proposal. The user also authorized the
[required post-P2 trial](P2_INTERVENTION_TRIAL.md); adoption remains optional. The original
...
| Required trial after P2 room | Evaluate one explicit pre-gravity intervention window under separate paused and timed profiles, using the trial plan's economy, episode and commit policies. Testing is required; adoption remains optional. Do not delay P1-A for it. |
...
No reactive command, new save field or mechanical timing policy is installed in
the current game by P1-A. The required trial's implementation defaults, tests and
adoption decision are specified in its separate plan.
