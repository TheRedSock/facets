...
**Current direction, 2026-09-22:** the implemented
[P3_READINESS_SUCCESSOR.md](P3_READINESS_SUCCESSOR.md) supersedes this plan's
atomic-only P3 direction. Its [final evidence](../tests/game/MERGE_FINAL_READINESS.md)
records repeated interventions, automatic-match redirection, default lookahead,
buffered input and measured engineering readiness on the declared profile.
The original closeout status, failed whole-action CPU target and earlier adoption
disposition below are historical; they are not a pending request for another
CPU waiver or human review. Broader human testing remains explicitly deferred.
