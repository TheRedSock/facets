...
**Later implementation:** [P1-A](P1A_HANDOFF.md) now restores concurrent normal
motion and records new release evidence. The original full corpus is preserved
at [the P1-A baseline copy](../artifacts/game/p1a/baseline/replay-corpus.json).
The measurements below remain the original P1 checkpoint, not current timings.

**Subsequent assessment:** the results below preserve the original functional
checkpoint. Presentation/performance acceptance is reopened in
[P1-A](P1_FLUIDITY_AMENDMENT.md). Serial per-cell playback is an identified
regression, not accepted final game feel. The
[fluidity review](../docs/FLUIDITY_AND_REACTIVE_PLAY.md) records a fresh diagnostic
and the missing acceptance criteria; it does not claim runtime fixes have landed.
...
Clarification from the subsequent review: the production player also omits the
old full-journey gravity curve/concurrency, so this is a visual-coherence defect
as well as excessive waiting. P1-A restores basic motion before P2 pacing review;
only broader decomposition/polish stays P4. The package burst above exercised gem
views and upgrade clips, not actual action computation/falling. Its frame p95
must not be cited as actual-action fluidity acceptance. Original measurements
