...
**Playback reconciliation, 2026-09-21:** P1-A/P2 deliver concurrent ordinary whole-journey motion with conservative wave barriers. Sequential playback below is the retained diagnostic reference/custom fallback, not the normal room default. New global dependencies must remain explicit. See `core/run/ROOM_CONTRACT.md`, `core/delivery/CONTRACT.md` and the current closeout journal.
