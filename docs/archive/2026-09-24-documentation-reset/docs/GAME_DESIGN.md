...
**Direction amendment, 2026-09-21:** the user selected repeated paid interventions
during every actionable merge animation, including moving a newly promoted gem
away from a pending automatic match into a stronger legal match or matching
elsewhere. Equilibrium retains unlimited thinking time; intervention windows
introduce optional reaction-based opportunity. This supersedes the atomic-only,
automatic-chain-first and no-reaction-opportunity assumptions below for the
proposed successor, while preserving the frozen P0/P1/P2 profiles. See
[the merge-window contract](../core/run/MERGE_WINDOW_PREPARATION.md) and
[P3-readiness plan](../plans/P3_READINESS_SUCCESSOR.md). One default automatic
merge or gravity result may be prepared privately and discarded on intervention.
The G0–G5 successor foundation is now implemented and checkpointed (2026-09-22),
including deterministic replay/restore. G6/G7 release and performance exits
remain open; follow [the execution journal](../plans/P3_READINESS_JOURNAL.md).
This does not establish gameplay quality. The user is the sole optional reviewer;
older trial/adoption and participant requirements below are historical.
...
[reactive-play assessment](FLUIDITY_AND_REACTIVE_PLAY.md) motivates the now
user-authorized [required post-P2 trial](../plans/P2_INTERVENTION_TRIAL.md),
comparing an untimed window with timed windows. Testing is committed; adopting
the mechanic into the main expedition remains optional. Costs, episode reward caps, input eligibility,
