...
**Current direction, 2026-09-22:** all seven [P3 implementation batches](../plans/P3_IMPLEMENTATION_PLAN.md) and required engineering exits are complete on the [declared profile](../tests/game/P3_FINAL_VERIFICATION.md). Families, four authored rooms, expedition progression, rewards/carry, disk Continue and integrated keyboard/mouse views build on the [merge-window successor](../plans/P3_READINESS_SUCCESSOR.md). The verification report links the one tested build and optional review form. The user is the sole optional reviewer; broader human evaluation is deferred, potentially until after P5. The original whole-action 5 ms failure is preserved. Historical status/dependency text below must not be read as a fresh measurement or as undoing the implemented P3 scope.

Status: P1-A concurrent motion and functional verification delivered, 2026-09-14; the 5 ms CPU acceptance gate remains open. The [P1-A handoff](../plans/P1A_HANDOFF.md) records results and recordings; the [P1-A amendment](../plans/P1_FLUIDITY_AMENDMENT.md) owns remaining acceptance work. The [game design](GAME_DESIGN.md) owns intended behavior. The [rewrite review](GAMEPLAY_REWRITE_REVIEW.md) and [fluidity assessment](FLUIDITY_AND_REACTIVE_PLAY.md) own source findings; [integration](GAME_ENGINE_INTEGRATION.md) and [art review](GEM_ART_REVIEW.md) own delivery/content work. The [content systems design](CONTENT_SYSTEMS.md) owns expansion semantics. The [original P1 handoff](../plans/P1_HANDOFF.md) remains historical evidence. This plan authorizes no automatic engine expansion.
...
Current execution route, 2026-09-21: the
[P2 finalization and P3 preparation plan](../plans/P2_FINALIZATION_AND_P3_PREPARATION.md)
starts with evidence preservation and an inherited-phase audit, then room
corrections, release/latency/player acceptance, the required intervention trial,
and P3 contract preparation. P2 is playable; its remaining gates are unchanged.
The review verified baseline/package file identity but did not rerun gameplay
checks or close any acceptance gate.
...
flowchart TD
    ...
    P1 --> P1A[P1-A Concurrent motion and latency repair]
    P1A --> P2[P2 One objective room and tactical tools]
    P2 --> Trial[Required intervention-window trial]
    Trial --> P3[P3 Families, rewards and three-room run]
...
ordered backlog and contract migration register. P1 now has a verified functional checkpoint; its simulation latency target remains unmet. The P0
...
- Record documentation/contract migrations needed by intentional mechanics changes.
  P0 originally listed AGENTS.md section edits; the subsequent root-guide rewrite
  is complete, and detailed behavior now belongs with subsystem owners when code
  lands. See the P0 contract migration register. Do not rewrite physical engine
  contracts to accommodate game preferences.
...
**Functional checkpoint completed 2026-09-14.** All eleven integrated stages,
100 seeds / 2,000 accepted actions with full replay and midpoint restore, clean
package audit/probe and actual release interactions pass. Fixed cost and bounded
playable openings were brought forward as approved. The 5 ms p95 target was
assessed and missed (131.716 ms headless complete-action p95). The
[implementation handoff](../plans/P1_HANDOFF.md) records exact coverage, remaining
performance work and P2/P3/P6 fixture ownership; it is not room acceptance.

**P1-A implementation:** normal ActionPlayer now animates ordinary column journeys
concurrently, with coherent acceleration, spaced refill and cancellation. The
serial reference remains available; custom topology uses conservative path
playback. All 2,000 original checkpoints, 12 integrated stages, the registered
GPU action probe and actual-release lifecycle checks pass. Release action frames
measure 8.496 ms p95; the separate complete-action corpus measures 39.907 ms p95
and misses the 5 ms target. Whole-wave match barriers remain; optional cross-wave
overlap is deferred. Player feel review and CPU acceptance remain open; see the
handoff for exact scope and evidence.
...
Implementation underway: B0–B5 are committed; B6 includes the accepted third sound set, workshop assets and package work. B7 automated release checks pass. The playable Open seam room is recorded in [P2_HANDOFF.md](../plans/P2_HANDOFF.md). Player sessions including dense in-game sound review, inherited latency/feel gates and the required intervention trial remain open.

The [detailed P2 implementation plan](../plans/P2_IMPLEMENTATION_PLAN.md) records
the current source gaps, proposed protocol decisions, ordered implementation
batches and regression/player acceptance. It is planning work, not a completed
P2 milestone; the inherited P1-A gates remain open.

Use the accepted P1-A motion baseline for pacing/playability review. Data/content
authoring can proceed earlier, but serial playback wait time must not be mistaken
for the tactical design's pacing. Separate decision time, forced waiting and
computation stalls in the first-room sessions.
...
Required experiment before this phase freezes: the user authorized the
[post-P2 intervention-window trial](../plans/P2_INTERVENTION_TRIAL.md).
Test one pre-gravity intervention in paused and timed profiles after the first
P2 room. Testing is committed work; adoption into the default expedition remains
optional. Its handoff must record a decision. Adoption requires versioned
episode/subcommand budgets, continuations, replay and save-boundary rules before
family/reward integration; otherwise retain atomic actions and stable-only saves.
...
- Split BoardScene into input/view/playback/overlay responsibilities while retaining pooled gem views, accepted P1-A concurrent motion, cancellation and the sequential diagnostic reference.
...
Repeat actual-action input/CPU/frame/playback measurements under full expedition
load. Gem-view burst performance alone does not establish game fluidity. Report
maximum stalls and forced waiting as well as p95. A timed reactive variant, if
adopted separately, needs its own timing, replay and accessibility acceptance.
...
Extend concurrent path/reservation and visual-causality tests to transport and
asymmetric rooms. Freely intercepting moving tiles or matching by logical arrival
time remains a separate scope decision, not implied by transport support.
...
The remaining P1-A acceptance work is **the complete-action CPU latency gate and player feel review**; concurrent motion and its functional/package checks are delivered. Preserve the current rule/identity baseline and recordings. P2's rubble-room content can proceed on this motion baseline, without declaring the unmet latency target complete. After P2, conduct the required paused/timed intervention-window trial and record an adoption decision before P3 contracts freeze.
