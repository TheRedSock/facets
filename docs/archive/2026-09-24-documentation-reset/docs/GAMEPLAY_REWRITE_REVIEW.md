...
**Historical source review, reconciled 2026-09-21:** source findings and recommendations below predate the delivered P1/P1-A/P2 implementations. Concurrent ordinary playback is now implemented; sequential mode is the diagnostic reference and conservative fallback. Current invariant audit and evidence are in [the closeout journal](../plans/P2_CLOSEOUT_JOURNAL.md); old gaps are not claims about current code.
...
| Root guides, subsystem guides and contracts | Keep ownership clear | Root AGENTS/README now route context; update detailed family/ladder/transaction behavior in owning subsystem documentation and tests as implementation lands |
...
## Expansion foundations added to the proposed rewrite

The 2026-09-14 [content systems design](CONTENT_SYSTEMS.md) clarifies the target architecture without changing the source findings above. Tier is the only appraisal axis; gameplay no longer needs a separate value table. Grade provenance justifies catalog placement outside simulation. Separate supply targets/weights from the ascending tier roster so an eventual high-tier supply setting does not invent conflicting match/value tiers.

Extend ordered rule events with immutable before/after definition snapshots, match shape/member ownership, assist classification, removal reasons and root/parent causes. One family reaction queue should consume typed facts and submit typed intents; it must not become a scene signal bus or arbitrary callback framework. Preserve coalesced movement paths for gravity/portal content. Add cause restrictions and scoped counters to save/hash as needed; suppression propagates to descendants.

Keep topology, gem occupancy, obstacle/lock state and floor overlays distinct. Later special occupants have explicit piece kinds instead of scattering T0/T9 exceptions through matching. Techniques/settings are explicit rule IDs; cosmetics resolve through presentation keys and cannot change the simulation hash. None of this requires implementing unused special pieces, every event handler or dynamic optical grades in the prototype.

The [build-plan scope table](PROTOTYPE_BUILD_PLAN.md#scope-guard-foundations-versus-content) assigns the minimal seams and tests to P0–P3. This revision is documentation only; the source probes and test counts below remain the earlier review's evidence, not a rerun.

## Final foundation audit before P1

The [architecture hardening](ARCHITECTURE_HARDENING.md) adds concrete planned
boundaries for typed rules, neutral UI language, workspace ownership, raw layout
admission, topology scheduling and versioned replay. Reinspection confirmed the
current in-place gravity scan has direction-dependent traversal behavior; an
optimization must not silently change it. Current JSON persistence also needs
explicit exact-integer encoding for RNG state, beyond merely adding missing fields.

The [non-gem asset specification](PRESENTATION_ASSETS.md) fills the previous gap
around background/UI/audio/VFX production and delivery. No source repair was made
as part of this final audit. Future editor work shares the same layout compiler
and admission service; it does not require an editor implementation in P1.

## Proposed contract updates, not silent violations

The new action-budget rule, roster-directed upgrades and complete-before-playback transaction supersede documented prototype behavior. Record them in the owning gameplay contracts/guides alongside implementation and regression updates. The rewritten root AGENTS.md is a routing guide; do not restore detailed mechanics there. Existing topological/rendering/RNG separation remains the baseline.
