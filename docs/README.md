...
|---|---|
| How do I set up another machine or rebuild missing assets? | [Fresh-machine build guide](BUILDING.md) |
...
`docs/` and `plans/` are versioned with source/contracts/tests. Generated assets
and raw review evidence are local; [setup and recovery](BUILDING.md) distinguishes
rebuildable outputs from historical evidence requiring a separate transfer. The
reorganization and preservation checks are described in
