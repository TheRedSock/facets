...
|---|---|---|
| Deterministic simulation and state identity | Preserve correctness guarantees, stable IDs, controlled RNG and complete replay/save identity | A chosen mechanic may change outcomes/order under a new version; retain old controls and provide a compatibility/regression plan |
...
| Current mechanics and scope | Open to critique and bounded alternative studies; keep the current playable baseline intact | Explicit selection before production migration, with player benefit, alternatives and timing/save/replay consequences |
...
Separate durable engineering invariants from revisable game rules. Current
families, tier silhouettes, room flow and merge-window policy are not invariants
merely because they have tests. A new design must be internally deterministic and
honestly versioned; it need not reproduce old outcomes if an intentional rules
change is selected. Do not rewrite old fixtures to conceal that change.
...
Collection/showroom views should share stable specimen/definition identity with
board views while owning separately budgeted larger assets. Avoid coupling a
layout, input device or render resolution to the rules. The future phone target
calls for measured, bounded runtime work and adaptable presentation, not a mobile
port or new platform abstraction framework in this stage.
