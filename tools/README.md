...
- **Production profile (`config/bake_profiles/gameplay.json`):** `default_environment` is `gameplay_studio_v2` (resolved via `production_bake_profile.gd` → tracer environment). The profile does **not** set `skip_stylize`, so production rotation atlases are **stylized** unless you add `"skip_stylize": true` to the JSON or bake via CLI with `--skip_stylize`.
