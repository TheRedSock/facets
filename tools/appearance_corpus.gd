...
func _run() -> void:
	...
	var options := {"cases":",".join(config.cases),"sizes":"112,256,512","profiles":"draft,detail,reference","frames":"0,1,2,3","rigs":",".join(config.rigs)}
	var plan_only := false
	...
	worker.release();print("CHECK_COMPLETE: appearance_corpus");quit()
