...
func _initialize() -> void:
	...
	if manifest.is_empty(): printerr("FAIL: cannot package realized specimens"); quit(1); return
	for key: String in manifest.jobs:
		...
		if saved.stone.get_meta("realization", {}).get("recipe_id") != String(recipe.recipe_id):
			printerr("FAIL: portable realization provenance missing"); quit(1); return
	...
	for index in stages.size():
		...
		if code != 0 or "SCRIPT ERROR:" in log_text or "ERROR:" in log_text:
			printerr("FAIL: portable specimen worker: " + log_text); quit(1); return
	...
	for job in jobs:
		if store.read(GemFramePlan.display_key(job)).is_empty() or store.read(GemGeometryPlan.key(job, 2)).is_empty():
			printerr("FAIL: missing realized specimen result"); quit(1); return
	...
	if library.is_empty() or int(library.statistics.frame_count) != jobs.size():
		printerr("FAIL: specimen delivery packing"); quit(1); return
