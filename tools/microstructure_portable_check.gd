...
func _initialize()->void:
	...
	if not realized.error.is_empty():printerr("FAIL: "+realized.error);print("CHECK_COMPLETE: microstructure_portable_check"); quit(1);return
	...
	if GemJobBundle.write(bundle,jobs,{},2).is_empty():printerr("FAIL: microstructure bundle");print("CHECK_COMPLETE: microstructure_portable_check"); quit(1);return
	...
	for stage in [PackedStringArray(["--headless","--editor","--quit"]),run,cached]:
		...
		var log_text:="\n".join(lines)
		if code!=0 or "SCRIPT ERROR:" in log_text or "ERROR:" in log_text:printerr("FAIL: microstructure portable "+log_text);print("CHECK_COMPLETE: microstructure_portable_check"); quit(1);return
	var store:=GemArtifactStore.new(output)
	for frame in jobs:
		if store.read(GemFramePlan.master_key(frame)).is_empty() or store.read(GemGeometryPlan.key(frame,2)).is_empty():printerr("FAIL: missing populated specimen outputs");print("CHECK_COMPLETE: microstructure_portable_check"); quit(1);return
	print("Microstructure portable PASS: three physical specimens/poses, geometry companions and headless cache reuse; "+base);print("CHECK_COMPLETE: microstructure_portable_check"); quit()
