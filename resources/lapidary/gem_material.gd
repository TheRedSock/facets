...
## Explicit measured or authored bulk scattering per millimeter.
...
func validate() -> PackedStringArray:
	...
	if not is_finite(scatter_per_mm) or scatter_per_mm < 0.0 or not is_finite(scatter_g) or absf(scatter_g) >= 1.0:
