...
**Objective:** demonstrate a coherent gemstone collection/build experience with
roguelike, match-3 and merge play, using physically grounded production and a
deliberate authored style. Evaluate the current three-room game as the tested
baseline; critique its design where the user's anchors warrant it. Prove the
selected direction before expanding production, retaining dependable engineering
guarantees through any explicitly chosen mechanical change.

## Sequence and current status

| Stage | Deliverable and exit | Status |
|---|---|---|
| P4.0 — establish the target | Gemology/collection/build concept critique, 2–3 distinct directions, board and collection/showroom studies, repeatable production evidence, scope recommendation and migration outline; select from concrete artifacts | Ready to start; user anchors clarified, no studies or direction selection completed |
| P4.1 — prove it in play | One representative playable segment and meaningful build choice, plus one gem's board/showroom treatment, actual new content, motion and sound; review ordinary and demanding sequences against the chosen target | Planned; depends on direction and any changed mechanical scope being selected |
...
| P4.3 — expand the proven treatment | Complete the selected prototype's screen/state/asset coverage, core controls/readability and a small presentation-separation proof; selected sources, manifests and package coverage | Planned; depends on a convincing study and usable foundations |
...
| Treat current mechanics and three-room structure as design constraints | User anchors permit conceptual critique; preserve P3 as a tested control and select any changed scope before migration |
| Treat photorealistic gem output as the goal | Develop physical rendering, lighting, print/style and surroundings together for a coherent final aesthetic |
| Keep collection inspection peripheral | Explore the connection between gemstone appreciation, collection choice and run build identity in the first study |
| Full alternate presentation profile as a P4 release requirement | Prioritize core experience; use a small diagnostic separation proof and defer broad profile/accessibility production |
...
Critique mechanics openly, including the current tier/collection relationship,
family model, room flow and reactive intervention policy. Tie alternatives to a
player benefit in the gemstone/roguelike/match-merge anchor. Do not require an
alternative to retain every current feature simply because it exists.

In a study retaining P3 mechanics, preserve its live post-merge input opportunity;
cinematic polish cannot silently alter it. A selected mechanic/protocol/scope
change gets explicit versioning, compatibility analysis and its own checks, with
the old controls retained. Exploration is authorized by the brief; production
migration awaits an actual choice of target, not a blanket rewrite mandate.
...
- Test art production as well as art direction: contrasting outputs and a
  controlled revision should support repeatability with available AI/tooling.
- Keep Windows first and basic controls intact. Design for future layout/input
  adaptation and assess the rough older-phone 60 fps intent without adding a
  mobile port, device certification or broad accessibility rollout to P4.
